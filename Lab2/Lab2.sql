...
-- 3
select t.TENANT_NAME, r.APT_NO from TENANT t LEFT JOIN RENTAL r on t.RENTAL_NO = r.RENTAL_NO;

-- 4
select s.CLUB_ID, s.NAME, c.AMOUNT from SPORTING_CLUBS s, CLUB_MEMBERSHIP c where s.CLUB_ID = c.CLUB_ID AND AMOUNT > 100;

-- 5
select c.FIRST_NAME, c.LAST_NAME, cm.CLUB_ID from customer c FULL JOIN club_membership cm on c.customer_id = cm.customer_id;
